...
bool GameMode::handle_event(SDL_Event const& e, glm::uvec2 const& window_size) {
	...
	// temporary mouse movement for camera
		...
		mouse.x = (e.motion.x + 0.5f) / float(2*640) * 2.0f - 1.0f;
		mouse.y = (e.motion.y + 0.5f) / float(2*400) * -2.0f + 1.0f;
		...
		for(auto const& person : Person::people){
			...
			BBox box = BBox(glm::vec3(pos.x-0.1,pos.y-0.1,0),
                            glm::vec3(pos.x+0.1,pos.y+0.1,2));
			if(box.intersects(r)){
				person->meshObject.transform.scale = glm::vec3(0.1,0.1,0.1);
